...
impl ClobClient {
    ...
    /// Execute a DELETE request with auth headers, body, and rate limiting.
    ///
    /// Accepts an optional body for cancel payloads (e.g. `{"orderID": ...}`).
    /// If body is empty, no Content-Type header is added.
    pub async fn delete(&self, path: &str, body: &str) -> Result<Response> {
        let url = format!("{}{}", self.config.base_url, path);
        ...
        self.execute_with_retry(request, "DELETE", path, body).await
    }
